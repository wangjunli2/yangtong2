...
int main(int argc, char *argv[])
{
    ...
    printf("input a number(3-5) and  the value of interval:(中间请用逗号隔开):");
    scanf("%d,%d", &N, &interval);
    while(interval < 1)
	{
		printf("please input a number(3-5) and value of interval again(中间请用逗号隔开):");
		scanf("%d,%d", &N, &interval);
	}
...
void    initSquare( SQUARE  *ps) 
{
    ...
    temp = NN;
    for(i=1;i<N;i++) {
        ps->threshold_max[N-1-i] = temp+ps->threshold_max[N-1-i+1];//求每行/列总和的最大值
        temp--;
    }
    ...
    temp = 1;
    for(i=1;i<N;i++) {
        ps->threshold_min[N-1-i] = temp+ps->threshold_min[N-1-i+1];//求每行/列总和的最小值
        temp++;
    }
